...
@if($edit == 'Tasker')
    @section('title', 'privacy-policy | Tasker')
@else
    @section('title', 'privacy-policy | User')
@endif
@section('content')
    ...
    <div class="boxShadow p-3 bgWhite m-b20">
        <div class="d-flex justify-content-between  flex-sm-row m-b25 services-new">
            <div>
                <h4 class="m-b25  blueTxtClr p-t5 p-b5">{{  $edit == 'Tasker' ? "Tasker" : "User" }} {{__('messages.Privacy Policy')}}</h4>
            </div>  
            <div class="col-6 col-sm-4 col-md-4 col-lg-2">
                <form action="{{ route('privacy.create', ['id' => $edit,'lang'=>'en' ]) }}" method="POST"  enctype="multipart/form-data">
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#status-selector").change(function(){
                ...
                if (edit == 'Tasker') {
                    window.location.href = "{{ route('privacy.create', ['id' => 'Tasker',''])}}"+"/"+lang;
                }
                else{
                    window.location.href = "{{ route('privacy.create', ['id' => 'User',''])}}"+"/"+lang;
                }
            });
        });
    </script>
