...
@section('content')
    <form class="boxShadow p-t20 p-b20 p-l15 p-r10 bgWhite m-b20" action="{{ route('bannerimage.store') }}" method="POST"  enctype="multipart/form-data">
        @csrf
        <h4 class="blueTxtClr p-t10 p-b10">{{trans('messages.Add')}} {{trans('messages.Banner')}}</h4>
        <div class="form-group">
            <label>{{trans('messages.image_url')}} </label> <span class="text-danger">*</span>  
            <input type="text"  class="form-control" name="banner_image_url" placeholder="{{trans('messages.image_url')}}" value="{{old('banner_image_url')}}" required>
