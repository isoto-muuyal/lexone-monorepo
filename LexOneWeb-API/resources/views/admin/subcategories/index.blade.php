...
@section('content')
    ...
    <div class="d-flex justify-content-between  flex-sm-row m-b25 services-new">
            <div>
            <h4 class="blueTxtClr p-t10 p-b10">{{trans('messages.Subcategories')}}</h4>
            </div>
            <div>
                <a href="{{ route('subcategory.add') }}">
                    <button class="btn btn-primary align-text-top border-0 m-b10">
                        ...
                        {{trans('messages.Add')}} {{trans('messages.Subcategory')}}
                    </button>
                </a>
            </div>
        </div>
        <div class="">
            <form method="GET" action="{{ route('subcategory.search') }}">
                        ...
                        @foreach($categoryrecords as $category)
                            <tr  class="text-center">
                                ...
                                <td class="fontSize15 text-center">
                                    <a href="{{ route('subcategory.show', ['subcategoryId' => $category['_id']]) }}" style="cursor: pointer;">
                                        <button class="btn btn-info align-text-top border-0"><i class="fa fa-eye" title="{{trans('messages.Show')}}"></i></button>
                                    </a>
                                </td>
                                <td class="fontSize15 text-center">
                                    <a href="{{ route('subcategory.edit', ['categoryId' => $category['_id'], ]) }}" style="cursor: pointer;">
                                        <button class="btn btn-info align-text-top border-0"><i class="fa fa-edit" title="{{trans('messages.Edit')}}"></i></button>
                                    </a>
                                </td>
                                <td class="fontSize15 text-center">
                                    @if($category['status'] == 0)
                                        <button class="btn btn-success align-text-top border-0"  data-toggle="modal" data-target="#exampleModalCenter-{{$category['_id']}}">
                                            <i class="fa fa-unlock" title="{{trans('messages.Enable')}}"></i>
                                        </button>
                                    @else
                                        <button class="btn btn-danger align-text-top border-0" data-toggle="modal" data-target="#exampleModalCenter-{{$category['_id']}}">
    ...
    <!-- modal -->
    @foreach($categoryrecords as $category)
        <div class="modal fade" id="exampleModalCenter-{{$category['_id']}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        @if($category['status'] == 0)
                             {{__('messages.Do you want to enable this Subcategory?')}}
                        @else
                            {{__('messages.Do you want to disable this Subcategory?')}} 
                        @endif
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        @if($category['status'] == 0)
                            <a href="{{ route('subcategory.activestatus', ['categoryId' => $category['_id'], 'categoryStatus' =>1 ]) }}" style="cursor: pointer;">
                                <button type="button" class="btn btn-success">{{__('messages.Enable')}}</button>
                            </a>
                        @else
                            <a href="{{ route('subcategory.activestatus', ['categoryId' => $category['_id'], 'categoryStatus' =>0 ]) }}" style="cursor: pointer;">
