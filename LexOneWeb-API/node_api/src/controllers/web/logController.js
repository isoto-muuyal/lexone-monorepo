...
let createLog =  async function (sender, receiver, reference, type, message, currency="default") {
    ...
    await Log.update({ messageTxt : message,bookingId : reference,receiverId : receiver }, logData, { upsert: true },function (error, logDetails) {
        if (error) {
            console.log('log_create_error',error);
        }
        else{
            console.log('create_logDetails',logDetails);
        }
    })
        ...
        try {
            ...
            userLogs.filter(function (eachLog) {
                ...
                allLogs.push({
                    ...
                    user_image: process.env.BASE_URL + process.env.TASKER_MEDIA_URL + eachLog.senderId.image,
