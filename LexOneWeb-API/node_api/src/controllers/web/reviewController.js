            ...
            if (req.body.review_id) {
                ...
                await newReview.save(function (error, reviewDetails) {
                    if (!error) {
                        ...
                        logController.createLog(userDetails._id, bookingDetails.taskerId, bookingDetails._id, "review", logMessage, currency);
        ...
        try {
            ...
            userReviews.filter(function (eachReview) {
                ...
                if (eachReview.bookingId){
                    var review_date = new Date(eachReview.updatedAt);
                    var month_name = monthNames[review_date.getMonth()];
                    var year = review_date.getFullYear();
                    var date = review_date.getDate();
                    var elapsed_date = date+'.'+month_name+'.'+year;
                    reviews.push({
...
let taskerRatings = async function (taskerId) {
    ...
    if (totalReviews) {
        ...
        console.log('taskerratings',taskerratings);
        if (taskerratings) {
