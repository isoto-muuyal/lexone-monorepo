...
const { tryMockLawyerSignIn } = require('../../config/mockAuth');
...
exports.signUp = async function (req, res) {
    if (!req.body.name || !req.body.email || !req.body.mobile || !req.body.password || !req.body.device_mode || !req.body.device_platform || !req.body.location || !req.body.language_type) {
        ...
        try {
            ...
            if (phoneExists)
                return res.status(200).json({ status_code: 400, message: res.__("Mobile number Already used by User") }); 
            ...
            await user.save(function (error, userDetails) {
                if (!error) {
                    ...
                    console.log(error);
                    return res.status(200).json({ status_code: 500, message: res.__("Something in went wrong") });
                }
            });
        }
        catch (err) {
            return res.status(200).json({ status_code: 500, message: res.__("Something out went wrong") });
        }
    }
};
...
exports.signIn = async function (req, res) {
    const mockSession = tryMockLawyerSignIn(req, userAuth);
    if (mockSession) {
        return res.status(200).json(mockSession);
    }

    //console.log('callsdfg',req.body.login_type)
    if (req.body.login_type === "phone") {
        userDetails = await User.findOneAndUpdate({ mobile: req.body.phone, role: "tasker" }, { deviceToken: req.body.device_token, deviceMode: req.body.device_mode, devicePlatform: req.body.device_platform, deviceActive: 1 })
        if (!userDetails)
...
exports.forgotPassword = async function (req, res) {
    ...
    if (!req.body.email) {
        ...
        try {
            ...
            if (userDetails.role === "user")
                return res.status(200).json({ status_code: 401, message: res.__("Enter tasker account email") });
...
//                     lon = parseFloat(req.body.source_lon);
//                     lat = parseFloat(req.body.source_lat);
...
//                             await userDetails.save();
//                         }else{
//                             return res.status(200).json({ status_code: 500, message: res.__("Phone number exist with another user") });
//                         }
//                     }
//                 }
...
exports.deleteAccount = async function (req, res) {
    ...
    if (!req.body.user_id) {
        ...
        try {
            console.log("goto TRY delete account API");
            let userDetails = await User.findOne({ userId: req.body.user_id, role: "tasker" });
            let bookingCount = await Booking.find({ taskerId: userDetails._id,bookedWhen: { $gt: new Date(moment().startOf('day').toISOString()) },$or: [{status:"accepted"},{status:"paid"},{status:"started"}]}).count();                       
            if(bookingCount>0)
            ...
            console.log("goto TRY2 delete account API");
            return res.status(200).json({
                status_code: 200,
...
exports.viewProfile = async function (req, res) {
    if (!req.params.taskerId) {
        ...
        try {
            ...
            taskerDetails.portfolio = mediaFiles;
            taskerDetails.completed_jobs = (userBookings && userBookings.length > 0) ? userBookings.length : 0;
    ...
    if (!req.params.taskerId) {
        ...
        try {
            ...
            if (userEarnings.length > 0) {
                totalEarnings = Math.round(userEarnings[0].total);
            }

            if (earningsPending.length > 0) {
                pendingEarnings = Math.round(earningsPending[0].total);
            }
    ...
    if (!req.params.taskerId) {
        ...
        try {
            ...
            if(userDetails.accountId != null){
                ...
                const site_url="https://tudofyapp.com/";
