...
let saveServices = async function (taskerId) {
    ...
    await User.findByIdAndUpdate(taskerId, { serviceIds: services }, {new: true}, function (error, result) {
        if (error) {
            console.log(error);
        }
        else{
...
let savePricing = async function (searchObject, saveObject, tasker_id, type) {
   let pricing =  await Pricing.findOneAndUpdate(searchObject,saveObject, { upsert: true }, async function (error, result) {
        if (error) {
