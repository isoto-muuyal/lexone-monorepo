...
exports.CategoryById = async function (req, res) {
    if(!req.body.category_id) {
        ...
        try {
            ...
            let categoryDetails = await Category.findById(req.body.category_id).exec();
            ...
            let category_info = [];
                var serviceName =  (categoryDetails[langName+'Name'])?(categoryDetails[langName+'Name']):categoryDetails.name ;
                category_info.push({
                    parent_category_id: categoryDetails._id,
                    parent_category_name:serviceName,
                    parent_category_image: process.env.BASE_URL + process.env.CATEGORY_MEDIA_URL + categoryDetails.image,
                    parent_category_type: categoryDetails.type,
                    location_type:categoryDetails.locationType,
                    category_faq : categoryFAQ,
                    category_description : categoryDescription,
                    about_category : aboutCategory
                });
                return res.status(200).json({ status_code: 200, category: category_info });
        }
        catch(err) {
            console.log(err);
...
exports.SubCategoryById = async function (req, res) {
    if(!req.body.sub_category_id) {
        ...
        try {
            ...
            let subcategoryDetails = await Subcategory.findById(req.body.sub_category_id).exec();
            let sub_category_info = [];
            var serviceName =  (subcategoryDetails[langName+'Name'])?(subcategoryDetails[langName+'Name']):subcategoryDetails.name ;
            sub_category_info.push({
                subcategory_id: subcategoryDetails._id,
                subcategory_name: serviceName,
                subcategory_image: process.env.BASE_URL + process.env.CATEGORY_MEDIA_URL + subcategoryDetails.image,
            });
            return res.status(200).json({ status_code: 200, category: sub_category_info });
        }
        catch(err) {
            console.log(err);
            return res.status(200).json({ status_code: 500, message: res.__("Something went wrong") });
...
exports.ServiceById = async function (req, res) {
    if(!req.body.service_id) {
        ...
        try {
            ...
            let serviceDetails = await Service.findById(req.body.service_id).exec();
            let service_info = [];
            var serviceName =  (serviceDetails[langName+'Name'])?(serviceDetails[langName+'Name']):serviceDetails.name ;
            service_info.push({
                service_id: serviceDetails._id,
                service_name: serviceName,
                service_image: process.env.BASE_URL + process.env.SERVICE_MEDIA_URL + serviceDetails.image,
                service_price: (serviceDetails.serviceCost) ? serviceDetails.serviceCost.toString() : "",
                service_pricing: res.__(serviceDetails.costType),
            });
            return res.status(200).json({ status_code: 200, category: service_info });
        }
        catch(err) {
            console.log(err);
            return res.status(200).json({ status_code: 500, message: res.__("Something went wrong") });
...
exports.Categories_service_tree = async function (req, res) {
    try {
        var host = req.headers['accept-language']; 
        let rawdata = fs.readFileSync('languageCode.json');
        var string = JSON.parse(rawdata);
        ...
        for (let index = 0; index < allCategories.length; index++) {
            ...
            var category_info = {};
            var CategoryName =  (category[langName+'Name'])?(category[langName+'Name']):category.name ;
            category_info.parent_category_id = category._id;
            category_info.parent_category_type = category.type;
            category_info.parent_category_name = CategoryName;
            // category_info.parent_category_image = process.env.BASE_URL + process.env.CATEGORY_MEDIA_URL + category.image;
            category_info.parent_category_id = category._id;
            let allSubCategories = await Subcategory.find({ status: 1, parentCategory: category._id });
            let subcategoryList = [];

            for (let index = 0; index < allSubCategories.length; index++) {
                const subcategory = allSubCategories[index];
                var sub_category_info = {};
                var subCategoryName =  (subcategory[langName+'Name'])?(subcategory[langName+'Name']):subcategory.name;
                sub_category_info.subcategory_id = subcategory._id;
                sub_category_info.subcategory_name = subCategoryName;
                // sub_category_info.subcategory_image = process.env.BASE_URL + process.env.CATEGORY_MEDIA_URL + subcategory.image;
                let serviceList = [];
    
                let allServices = await Service.find({ status: 1, mainCategory: category._id, subCategory: subcategory._id });   
                
                for (let index = 0; index < allServices.length; index++) {
                    const services = allServices[index];
                    var service_info = {};

                    var serviceName =  (services[langName+'Name'])?(services[langName+'Name']):services.name ;

                    service_info.service_id = services._id;
                    service_info.service_name = serviceName;
                    // service_info.service_image = process.env.BASE_URL + process.env.SERVICE_MEDIA_URL + services.image;
                    service_info.service_price = (services.serviceCost) ? services.serviceCost.toString() : "";
                    service_info.service_pricing = res.__(services.costType);
                    serviceList.push(service_info);
                }
                sub_category_info.services = serviceList;
                subcategoryList.push(sub_category_info);
            }

            category_info.subcategories = subcategoryList;
            categoryList.push(category_info);
        }


        return res.status(200).json({ status_code: 200, category_tree: categoryList });
    }
    catch (err) {
        console.log(err);
        return res.status(200).json({ status_code: 500, message: res.__("Something went wrong") });
...
exports.allServices = async function (req, res) {
    if (!req.body.parent_category_id || !req.body.subcategory_id) {
        return res.status(200).json({ status_code: 400, message: res.__("Invalid Params") });
    }
    else {
        try {
            ...
            if(allServices.length > 0) {
                if (host == "en") {
                    allServices.filter(function (eachService) {
                        ...
                        if(eachService.mainCategory != null) {
                            searchSuggestions.push({
                                parent_category_id: eachService.mainCategory && eachService.mainCategory._id === null ? '' : eachService.mainCategory._id,
