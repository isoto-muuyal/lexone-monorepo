...
const lexoneLogo = require("../assets/images/lexone-logo.png");
...
class Header extends React.Component {
    ...
    logo_image_err = (ev) => {
        var general_info = JSON.parse(localStorage.getItem('general_info'));
        if (general_info && general_info.site_logo) {
    ...
    render() {
        ...
        const showSearch = (this.props.user && this.props.user.type === 'user') || !this.props.user;

        return (
            <nav className={`topNav${isHome ? ' lexone-nav' : ''}`} id="area">
                    ...
                    <div className="d-flex header">
                        ...
                        <div className="align-self-center ml-auto">
                            <Link to="/user/user-login">
                                <Button className={`PrimaryBtn lg${isHome ? ' lexone-account-btn' : ''}`}> {i18next.t('Account')} </Button>
                            </Link>
                        </div> : [
                            (
