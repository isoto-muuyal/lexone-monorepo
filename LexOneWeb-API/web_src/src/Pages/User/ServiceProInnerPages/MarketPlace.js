...
class MarketPlace extends Component {
    ...
    validate_market_service = (id) => {
        var err = 0;

        if(!this.state.user_info.mobile) {
            this.setState({is_phone_number_ModalVisible : true, update_mobile_number : '', update_mobile_number_err : '' });
            err++;
        }
        if(err === 0) {
            var allocated_services = this.state.allocated_services;
            var chosen_service = allocated_services.filter(x => x.service_id === id);
           
            this.props.history.push('/user/booking', chosen_service);
        }
    }
    push_to_login_page = () => {
        var obj = {};
    ...
    firebaseui_loads() {
        ...
        this.state.uiconfig_state = uiconfig;
    }
    save_mobile_number = () => {
        const config = {
            headers: {
                            ...
                            <div className="row">
                                    ...
                                    <section className="">
                                        {
                                            ...
                                            <div className="border roundedFivePx">
                                                            ...
                                                            <div className="pr-2 mr-auto">
                                                                <div className="details">
                                                                    ...
                                                                    <div className="detailTwo d-flex">
                                                                        <div className="singleStar align-self-center"><Rate disabled allowHalf defaultValue = {i.rating} /> </div>
                                                                        <span className="align-self-center m-t-5 ml-2"> {i.rating}</span>
                                                                    </div>
                                                ...
                                                <div className="m-3">
                                                    <button onClick={ (e)=> { e.stopPropagation(); this.setState({ review_page : this.state.review_page + 1 }); this.get_reviews_by_category_id();  }  } type="button" className="loadMoreServices border-0 w-100">{i18next.t('Load more')} </button>
                                                </div>
                                            </div>
                                            :
                                ...
                                <div className="col-lg-5 col-md-12 mt-lg-0 mt-4">
                                    ...
                                    <section className="rightSideBox">
                                        ...
                                        <div>
                                            <div className="content" style={{ maxHeight: '60vh', overflowX: 'hidden' }}>
                                                {
                                                this.state.allocated_services &&
                                                this.state.allocated_services.length > 0 ?
                                                this.state.allocated_services.map((i) => {
                                                    
                                                    return (
                                                        <div onClick={ (e)=>{ e.stopPropagation(); if(this.state.user_info) { this.validate_market_service(i.service_id) } else { this.push_to_login_page(); }  } } key={i.service_id} className="hover-cont">
                                                            <div className="row">
                                    ...
                                    <div className="mt-4">
                                            {/* <Button type="button" onClick={ this.state.user_info ? this.validate_market_service : this.push_to_login_page } className="d-flex justify-content-between align-items-center Btns PrimaryBtn lg fM" block><span>{this.state.general_info.currency_symbol} {this.state.sum_of_service_charge}</span><span>{ this.state.user_info ? 'Next' : 'Login' }</span></Button>
                                    ...
                                    <Button onClick={this.save_phone_number_modal_close}>{i18next.t('Close')}</Button>
                                    ]}>
