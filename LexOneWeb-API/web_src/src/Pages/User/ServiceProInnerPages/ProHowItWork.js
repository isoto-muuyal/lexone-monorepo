                    ...
                    <div className="innerPage">
                        <div className="container">
                            <div className="d-flex flex-wrap">
                                ...
                                <button type="button" onClick={ (e) => { e.stopPropagation(); this.show_blocks_based_on_flag('how_it_works'); } } className={ this.state.howit_active === true ? 'active' : '' }><p className="mb-0 ">{ i18next.t('How It Work') }</p></button>

                                <button type="button" onClick={ (e) => { e.stopPropagation(); this.show_blocks_based_on_flag('about'); } } className={ this.state.about_active === true ? 'active' : '' }><p className="mb-0">{ i18next.t('About') }</p></button>
                                {  
                                    this.state.category_info && this.state.category_info.parent_category_type === 'professional' &&

                                    <button type="button" onClick={ (e) => { e.stopPropagation(); this.show_blocks_based_on_flag('review'); } } className={ this.state.review_active === true ? 'active' : '' } ><p className="mb-0">{ i18next.t('Review') }</p></button>
                                }

                                <button type="button" onClick={ (e) => { e.stopPropagation(); this.show_blocks_based_on_flag('faq'); } } className={ this.state.faq_active === true ? 'active' : '' }><p className="mb-0">{ i18next.t('FAQ') }</p></button>
                            </div>
                        </div>
                    </div>
                    </section>
